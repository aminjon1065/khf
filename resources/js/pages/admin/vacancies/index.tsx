import { Head, Link, router } from '@inertiajs/react';
import { Pencil, Plus, Trash2 } from 'lucide-react';
import { useState } from 'react';
import { DataTable } from '@/components/admin/data-table';
import type {
    DataTableColumn,
    DataTableFilters,
    Paginator,
} from '@/components/admin/data-table';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import {
    Dialog,
    DialogContent,
    DialogDescription,
    DialogFooter,
    DialogHeader,
    DialogTitle,
} from '@/components/ui/dialog';
import { dashboard } from '@/routes/admin';
import { create, destroy, edit, index, trash } from '@/routes/admin/vacancies';

type VacancyRow = {
    id: number;
    title: string;
    department: string | null;
    employment_type: string;
    employment_type_label: string;
    status: string;
    status_label: string;
    positions_count: number;
    applications_count: number;
    locales: string[];
    published_at: string | null;
    deadline_at: string | null;
    is_open: boolean;
};

type PageProps = {
    vacancies: Paginator<VacancyRow>;
    filters: DataTableFilters;
    trashedCount: number;
};

const statusVariant: Record<
    string,
    'default' | 'secondary' | 'outline' | 'destructive'
> = {
    published: 'default',
    draft: 'secondary',
    moderation: 'outline',
    archived: 'destructive',
};

export default function VacanciesIndex({
    vacancies,
    filters,
    trashedCount,
}: PageProps) {
    const [deleting, setDeleting] = useState<VacancyRow | null>(null);

    const columns: DataTableColumn<VacancyRow>[] = [
        {
            key: 'title',
            label: 'Должность',
            render: (vacancy) => (
                <div>
                    <span className="font-medium">{vacancy.title}</span>
                    {vacancy.department && (
                        <span className="block text-xs text-muted-foreground">
                            {vacancy.department}
                        </span>
                    )}
                </div>
            ),
        },
        {
            key: 'locales',
            label: 'Языки',
            render: (vacancy) => (
                <div className="flex gap-1">
                    {['tj', 'ru', 'en'].map((locale) => {
                        const hasTranslation = vacancy.locales.includes(locale);

                        return (
                            <Badge
                                key={locale}
                                variant={hasTranslation ? 'default' : 'outline'}
                                className={
                                    hasTranslation
                                        ? 'px-1.5 text-[10px] uppercase'
                                        : 'px-1.5 text-[10px] text-muted-foreground uppercase'
                                }
                            >
                                {locale}
                            </Badge>
                        );
                    })}
                </div>
            ),
        },
        {
            key: 'employment_type',
            label: 'Занятость',
            className: 'hidden md:table-cell',
            render: (vacancy) => vacancy.employment_type_label,
        },
        {
            key: 'applications_count',
            label: 'Заявки',
            className: 'hidden lg:table-cell',
            render: (vacancy) => vacancy.applications_count,
        },
        {
            key: 'deadline_at',
            label: 'Срок подачи',
            sortable: true,
            className: 'hidden sm:table-cell',
            render: (vacancy) =>
                vacancy.deadline_at ? (
                    <span className={vacancy.is_open ? '' : 'text-destructive'}>
                        {vacancy.deadline_at}
                    </span>
                ) : (
                    '—'
                ),
        },
        {
            key: 'status',
            label: 'Статус',
            sortable: true,
            render: (vacancy) => (
                <Badge variant={statusVariant[vacancy.status] ?? 'secondary'}>
                    {vacancy.status_label}
                </Badge>
            ),
        },
    ];

    return (
        <>
            <Head title="Вакансии" />

            <div className="flex h-full flex-1 flex-col gap-6 p-4">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-semibold">Вакансии</h1>
                        <p className="text-sm text-muted-foreground">
                            Вакантные должности государственной службы
                        </p>
                    </div>
                    <Button variant="outline" size="sm" asChild>
                        <Link href={trash().url}>
                            <Trash2 className="size-4" />
                            Корзина {trashedCount > 0 && `(${trashedCount})`}
                        </Link>
                    </Button>
                </div>

                <DataTable
                    columns={columns}
                    paginator={vacancies}
                    filters={filters}
                    baseUrl={index().url}
                    getRowId={(vacancy) => vacancy.id}
                    searchPlaceholder="Поиск по названию должности…"
                    toolbar={
                        <Button asChild>
                            <Link href={create().url}>
                                <Plus className="size-4" />
                                Создать вакансию
                            </Link>
                        </Button>
                    }
                    actions={(vacancy) => (
                        <div className="flex justify-end gap-1">
                            <Button
                                variant="ghost"
                                size="icon"
                                aria-label="Изменить"
                                asChild
                            >
                                <Link href={edit(vacancy.id).url}>
                                    <Pencil className="size-4" />
                                </Link>
                            </Button>
                            <Button
                                variant="ghost"
                                size="icon"
                                aria-label="Удалить"
                                onClick={() => setDeleting(vacancy)}
                            >
                                <Trash2 className="size-4" />
                            </Button>
                        </div>
                    )}
                />
            </div>

            <Dialog
                open={Boolean(deleting)}
                onOpenChange={(open) => !open && setDeleting(null)}
            >
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        <DialogTitle>Удалить вакансию?</DialogTitle>
                        <DialogDescription>
                            «{deleting?.title}» будет перемещена в корзину.
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
                        <Button
                            variant="outline"
                            onClick={() => setDeleting(null)}
                        >
                            Отмена
                        </Button>
                        <Button
                            variant="destructive"
                            onClick={() => {
                                if (deleting) {
                                    router.delete(destroy(deleting.id).url, {
                                        preserveScroll: true,
                                        onSuccess: () => setDeleting(null),
                                    });
                                }
                            }}
                        >
                            В корзину
                        </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </>
    );
}

VacanciesIndex.layout = {
    breadcrumbs: [
        { title: 'Панель управления', href: dashboard() },
        { title: 'Вакансии', href: index() },
    ],
};
